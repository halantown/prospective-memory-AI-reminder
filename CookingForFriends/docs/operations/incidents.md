...
## INC-008 — Hard-coded DEV_TESTER startup seed crashes backend
...
| Severity     | P1 High |
| Status       | Resolved |
| Reported by  | Backend startup error log |
| Affected area| Backend lifespan startup, development/test participant creation |

### Background
> Participant login tokens are 6-character tokens generated by Admin participant creation or `/api/admin/test-session`. Backend startup should not create special participant identities.

### Incident Description
> Backend startup failed during FastAPI lifespan initialization with a PostgreSQL unique constraint violation: `participants_participant_id_key`, `participant_id=DEV_TESTER`. The application exited before serving requests. The deeper issue was that startup contained a hard-coded reusable `DEV_TESTER` participant path that bypassed the normal generated-token workflow.

### Timeline
| Time (local) | Event |
|--------------|-------|
| 20:02 | Startup failed with duplicate `DEV_TESTER` unique constraint error |
| 20:03 | Investigation started in `database.py` startup seed logic |
| 20:03 | Root cause identified: seed lookup only checked token, not `participant_id` |
| 20:05 | Initial idempotency fix implemented |
| 20:12 | Hard-coded `DEV_TESTER` / `DEV_TOKEN` startup seed removed |
| 20:13 | Confirmed with backend tests and startup import checks |

### Root Cause
> `seed_dev_participant()` hard-coded `participant_id='DEV_TESTER'` and was called unconditionally during startup. The seed lookup originally used `Participant.token == DEV_TOKEN`, so a database with an existing `DEV_TESTER` row and a different token caused a duplicate insert. More broadly, the startup seed was redundant with Admin/test-session token generation and introduced a special-case identity that did not match the participant login workflow.

### Contributing Factors
> Development tokens can change across local runs while the database persists. Documentation also encouraged `DEV_TOKEN=ABC123`, which made the special startup path look like a supported testing mechanism even though normal participant tokens are generated through Admin flows.

### Fix
> Removed the `DEV_TOKEN` / `DEV_TESTER` startup seed path. `backend/main.py` no longer imports or calls `seed_dev_participant()`, `backend/database.py` no longer contains the seed function, and `backend/config.py` no longer exposes `DEV_TOKEN`. Development testing now uses generated 6-character tokens from Admin participant creation or `/api/admin/test-session`.

### Verification
> Backend test suite was run after the fix. Code search confirmed there are no remaining active `DEV_TESTER`, `DEV_TOKEN`, or `seed_dev_participant` references outside historical incident text.

### Follow-up Actions
> - [ ] Add a lightweight startup regression check that verifies no participant rows are created during backend lifespan initialization.
