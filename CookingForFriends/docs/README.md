# CookingForFriends Docs

Use this as the only documentation entry point. Current docs are grouped by
audience.

## Current Docs

| Need | Document |
|------|----------|
| System shape and runtime ownership | [development/architecture.md](development/architecture.md) |
| Local setup, env vars, runtime knobs | [development/configuration.md](development/configuration.md) |
| Manual QA and test flow | [development/testing.md](development/testing.md) |
| PM doorbell / phone-call flow | [development/pm-trigger-flow.md](development/pm-trigger-flow.md) |
| Cooking ongoing task and kitchen assets | [development/cooking.md](development/cooking.md) |
| Incident log and post-mortem template | [operations/incidents.md](operations/incidents.md) |
| PM task storyboard and script materials | [research/storyboard/pm_tasks/pm_tasks_design.md](research/storyboard/pm_tasks/pm_tasks_design.md) |

## Directory Rules

- `development/`: implementation docs that should match current code.
- `operations/`: incident logs, runbooks, and operational records.
- `research/`: study materials, storyboards, scripts, and assets.
Keep this README small. Add details to the relevant supporting document instead
of creating more root-level docs.
