...
- 剩余风险：
  - 这仍然是复制 backend recipe 到 frontend，不是最终架构。
  - 后续改 recipe 时仍可能忘记同步 frontend。
  - 更合理的方案是 backend 在 session/bootstrap 时下发 recipe definitions，frontend 只渲染 server-provided recipe。
- 理想架构决策：
...
- 后续建议：
  - [ ] 新增 session/bootstrap payload，返回完整 backend recipe definitions 和 recipe version。
  - [ ] 删除 frontend `gameStore.ts` 中的 recipe hardcode。
  - [ ] 前端 `RecipeTab` 改为只渲染 bootstrap recipes + runtime state，不再拥有 recipe 内容定义。
